...
class Tmdb::WatchProvider
  ...
  def self.available_regions(language : String? = nil, skip_cache : Bool = false) : Array(Region)
  ...
  def self.movie_providers(
    ...
    res.get(skip_cache)["results"].as_a.map { |provider| Provider.new(provider) }
  end

  # Returns a list of the watch provider (OTT/streaming) data we have
  # available for TV. You can specify a `watch_region` param if
  # you want to further filter the list by country.
  def self.tv_providers(
    language : String? = nil,
    watch_region : String? = nil,
    skip_cache : Bool = false
  ) : Array(Provider)
